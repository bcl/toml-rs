...
impl ser::Serialize for Datetime {
        ...
        where S: ser::Serializer
    {
        ...
        let mut s = serializer.serialize_struct("$__toml_private_Datetime", 1)?;
        s.serialize_field("$__toml_private_datetime", &self.to_string())?;
        s.end()
    }
}
        ...
        where D: de::Deserializer
    {
        ...
        impl de::Visitor for DatetimeVisitor {
            ...
            fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
                formatter.write_str("a valid TOML datetime")
            }

            fn visit_map<V>(self, mut visitor: V) -> Result<Datetime, V::Error>
                where V: de::MapVisitor
            {
                let value = visitor.visit_key_seed(DatetimeFromStringKey)?;
...
struct DatetimeFromStringKey;

impl de::DeserializeSeed for DatetimeFromStringKey {
    type Value = ();
        ...
        where D: de::Deserializer
    {
        ...
        deserializer.deserialize_u32(FieldVisitor)
    }
}
        ...
        impl de::Visitor for Visitor {
                ...
                where E: de::Error,
            {
                match s.parse() {
                    ...
                    Err(_e) => {
                        let msg = format!("failed to parse date");
