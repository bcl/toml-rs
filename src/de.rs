...
#[derive(Debug, Clone)]
pub enum ErrorKind {
    /// Indicates that a Rust type was requested to be deserialized but it was
    /// not supported.
    ///
    /// Currently the TOML format does not support deserializing types such as
...
impl<'a, 'b> de::SeqVisitor for MapVisitor<'a, 'b> {
        ...
        where K: de::DeserializeSeed,
    {
        ...
        let next = next.unwrap_or(self.max);
...
impl<'a> Deserializer<'a> {
    ...
    // TODO: shouldn't buffer up this entire table in memory, it'd be great to
    // defer parsing everything until later.
    ...
    // TODO: shouldn't buffer up this entire array in memory, it'd be great to
    // defer parsing everything until later.
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.inner.kind {
            ErrorKind::UnsupportedType => "unsupported Rust type".fmt(f)?,
...
impl error::Error for Error {
    fn description(&self) -> &str {
        match self.inner.kind {
            ErrorKind::UnsupportedType => "unsupported Rust type",
